"""Support for Google Actions Smart Home Control (secondary instance)."""
...
from homeassistant.components.google_assistant.smart_home import async_handle_message
...
class GoogleConfigSecondary(AbstractConfig):
    """Config for the secondary Google Assistant instance."""

    _store: GoogleConfigStoreSecondary
    ...
    async def async_initialize(self):
        ...
        self._store = GoogleConfigStoreSecondary(self.hass)
    ...
    def async_enable_local_sdk(self) -> None:
        """Enable the local SDK, registering webhooks under the secondary domain."""
        _LOGGER.debug("async_enable_local_sdk (secondary)")
        ...
        for user_agent_id in self.async_get_agent_users():
            ...
            try:
                webhook.async_register(
                    self.hass,
                    DOMAIN,
                    "Local Support (Secondary) for " + user_agent_id,
                    webhook_id,
...
class GoogleConfigStoreSecondary:
    """A configuration store for the secondary google assistant integration."""
...
class GoogleAssistantSecondaryView(HomeAssistantView):
    """Handle Google Assistant requests for the secondary integration."""

    url = GOOGLE_ASSISTANT_API_ENDPOINT
    name = "api:google_assistant_secondary"
    requires_auth = True
