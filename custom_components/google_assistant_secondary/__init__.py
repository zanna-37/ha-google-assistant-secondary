"""Support for a secondary Google Assistant Smart Home integration."""
...
from .http import GoogleAssistantSecondaryView, GoogleConfigSecondary
...
type GoogleSecondaryConfigEntry = ConfigEntry[GoogleConfigSecondary]
...
async def async_setup(hass: HomeAssistant, yaml_config: ConfigType) -> bool:
    """Activate the secondary Google Actions component."""
...
async def async_setup_entry(
    hass: HomeAssistant, entry: GoogleSecondaryConfigEntry
) -> bool:
    ...
    device_registry.async_get_or_create(
        ...
        manufacturer="Google",
        model="Google Assistant (Secondary)",
        name=config[CONF_PROJECT_ID],
        entry_type=dr.DeviceEntryType.SERVICE,
    )

    google_config = GoogleConfigSecondary(hass, config)
    await google_config.async_initialize()

    entry.runtime_data = google_config

    hass.http.register_view(GoogleAssistantSecondaryView(google_config))
