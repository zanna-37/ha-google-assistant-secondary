"""Support for sync button (secondary integration)."""
...
from . import GoogleSecondaryConfigEntry
from .const import CONF_PROJECT_ID, CONF_SERVICE_ACCOUNT, DATA_CONFIG, DOMAIN
from .http import GoogleConfigSecondary


async def async_setup_entry(
    hass: HomeAssistant,
    config_entry: GoogleSecondaryConfigEntry,
...
class SyncButton(ButtonEntity):
    ...
    def __init__(self, project_id: str, google_config: GoogleConfigSecondary) -> None:
        """Initialize button."""
        super().__init__()
        self._google_config = google_config
        self._attr_entity_category = EntityCategory.DIAGNOSTIC
        self._attr_unique_id = f"{project_id}_sync_secondary"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, project_id)},
            name="Google Assistant (Secondary)",
        )
